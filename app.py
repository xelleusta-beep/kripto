...
def get_crypto_data(symbol_name, safe_limit, inv_str):
    try:
        ...
        safe_limit = min(max(int(safe_limit), 30), 1500)
...
def compute_strategy_performance(df_input, train_ratio, init_cash=10000.0):
    try:
...
def process_live_alarms(b_token, c_id):
    ...
    for alarm in st.session_state.alarms:
        ...
        if alarm_raw.empty or len(alarm_raw) < 10:
            continue
        try:
            res = compute_strategy_performance(alarm_raw, 80, alarm["balance"] if alarm["balance"] > 0 else 10000.0)
            if res is not None:
                ...
                if alarm["last_signal"] != a_signal:
                    ...
                    if a_signal == 1 and alarm["balance"] > 0:
                        ...
                        action = (
                            ...
                            f"💵 *Harcanan:* `${alarm['crypto_amount'] * a_price:,.2f}`\n"
                        ...
                        pnl_usd = (alarm["crypto_amount"] * a_price) - (alarm["crypto_amount"] * entry_price)
                        pnl_pct = ((a_price - entry_price) / entry_price * 100) if entry_price > 0 else 0.0
...
if st.sidebar.button("🚨 SEÇİLİ COİNİ ALARMLARA EKLE", width="stretch"):
    if not any(a["ticker"] == ticker and a["interval"] == interval_mapping[interval_label] for a in st.session_state.alarms):
        ...
        st.sidebar.warning("Bu alarm zaten mevcut!")
...
processed_df, total_net_return_pct, final_wallet_value, backtest_logs, latest_signal = compute_strategy_performance(raw_df, train_size, backtest_balance)
...
**İşlem Kuralları:**
1. Model **1** dönerse ve pozisyonda değilse → **🟢 ALIM** (tüm bakiye ile, %0.02 taker fee)
2. Model **0** dönerse ve pozisyondaysa → **🔴 SATIM** (nakde geçiş, %0.02 taker fee)
3. Pozisyon yokken 0 gelirse → **bekleme** (işlem yapılmaz)
