...
st.set_page_config(layout="wide", page_title="Yapay Zeka, Paper Trading & Telegram")
st.title("🤖 ML Backtest, Paper Trading & Telegram Sinyal Platformu")
...
if "last_sent_signal" not in st.session_state:
    st.session_state.last_sent_signal = None  # Aynı sinyalin tekrar tekrar Telegram'a gitmesini engeller
...
# --- YENİ ÖZELLİK: AYARLAR VE TELEGRAM ENTEGRASYONU ---
st.sidebar.markdown("---")
st.sidebar.header("⚙️ Telegram Sinyal Ayarları")

# Sinyalleri aktif/pasif etme butonu
telegram_active = st.sidebar.toggle("Telegram Sinyallerini Aktif Et", value=False)
...
st.write(f"### 📈 {ticker} ({time_period} boyunca {interval_label} Verileri) Analiz Raporu")
...
try:
    ...
    df = pd.DataFrame({
        'Open': yf_data.get('Open'), 'High': yf_data.get('High'), 'Low': yf_data.get('Low'), 'Close': yf_data.get('Close')
    })
    ...
    if df.empty or len(df) < 30:
        st.error("Yetersiz veri! Lütfen zaman aralığı ayarlarını değiştirin.")
    else:
        # 4. Feature Engineering (ML için İndikatörler)
        ...
        # --- TELEGRAM SİNYAL TETİKLEYİCİSİ ---
        if telegram_active and bot_token and chat_id:
            # Sinyal değiştiyse veya ilk kez sinyal gönderiliyorsa tetikle
            if st.session_state.last_sent_signal != latest_signal:
                signal_text = "🟢 AL (BUY)" if latest_signal == 1 else "🔴 SAT (SELL)"
                msg = (
                    f"🤖 *YAPAY ZEKA YENİ SİNYALÜ*\n\n"
                    f"▪️ *Varlık:* {ticker}\n"
                    f"▪️ *Sinyal:* {signal_text}\n"
                    f"▪️ *Anlık Fiyat:* ${current_price:,.2f}\n"
                    f"▪️ *Zaman Sıklığı:* {interval_label}\n"
                    f"▪️ *Test Aralığı:* {time_period}\n"
                )
                success = send_telegram_signal(bot_token, chat_id, msg)
                if success:
                    st.session_state.last_sent_signal = latest_signal
                    st.sidebar.success("Sinyal başarıyla Telegram'a gönderildi!")
                else:
                    st.sidebar.error("Telegram mesajı gönderilemedi! Bilgileri kontrol edin.")

        # 6. Vectorbt Backtest Simülasyonu
        portfolio = vbt.Portfolio.from_signals(
            df['Close'], entries=(df['Predicted_Signal'] == 1), exits=(df['Predicted_Signal'] == 0), fees=0.001
        )
        ...
        with tab2:
            ...
            p_col2.metric(f"Varlık Miktarı ({ticker.split('/')})", f"{st.session_state.crypto_amount:.4f}")
    ...
    st.error(f"Veri çekme veya işlem hatası: {e}")
